...
We are basically fitting all the data that we are loading into our model by "fit" method.

## Model Evaluation

After training all the data, we are evaluating the models in different aspect:

# Accuracy using evaluate() method

We are trying to see the accuracy score that is returned from the evaluate() method so we can see if the accuracy score is high enough or not.
